...
if(!((Life_veh_shop select 2) in ["admin","don"])) then
{
...
if((Life_veh_shop select 2) == "admin") then // test securité
{
	for "_i" from 0 to count(_colorArray)-1 do 
	{
		//if((_colorArray select _i) select 1 == (life_veh_shop select 2))then {
...
if((Life_veh_shop select 2) == "don") then // test securité
{
	for "_i" from 0 to count(_colorArray)-1 do 
	{
		if((_colorArray select _i) select 1 in ["civ","reb","donate","jour","fed"]) then 
		{
			_temp = [_className,_i] call life_fnc_vehicleColorStr;
			_ctrl lbAdd format["%1",_temp];
			_ctrl lbSetValue [(lbSize _ctrl)-1,_i];
