...
if(_curTarget isKindOf "House_F" && {player distance _curTarget < 12} OR ((nearestObject [[12426,14098.7,0],"Land_Offices_01_V1_F"]) == _curTarget)) exitWith { //[16019.5,16952.9,0],"Land_Dome_Big_F" OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"] == _curTarget))
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if(_isVehicle) then {
		...
		if((typeOf _curTarget) in _animalTypes) then {
			if((typeOf _curTarget) == "Turtle_F" ) then { //&& !alive _curTarget
				private["_handle"];
