...
switch (_code) do
{
	...
	//	hint "Le téléphone de la personne est au sol.";
	//	_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);}
	//else { hint "La personne que vous avez mis à terre n'a plus son téléphone!"};
	//		};
	//	};
	//};
	...
	case 31:
	{
			if ((BipBipOn) and (vehicle player != player && (typeOf vehicle player) in ["C_Van_01_transport_F","C_Van_01_box_F","I_Truck_02_transport_F","I_Truck_02_covered_F","B_Truck_01_transport_F","O_Truck_03_transport_F","O_Truck_03_covered_F","B_Truck_01_box_F"])) then {
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","C_Hatchback_01_F","I_MRAP_03_F","B_Heli_Light_01_F","B_MRAP_01_hmg_F","B_Heli_Transport_01_F","O_Heli_Light_02_unarmed_F","C_Heli_Light_01_civil_F","B_Heli_Transport_03_unarmed_F"]) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
                        ...
                        [[player, "car_unlock",15],"life_fnc_playSound",true,false] spawn life_fnc_MP;
					} else {
						if(local _veh) then {
							_veh lock 2;
						} else {
