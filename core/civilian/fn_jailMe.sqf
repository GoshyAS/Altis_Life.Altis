...
_time = [_this,2,15,[0]] call BIS_fnc_param; //##80 Ajout

_time = time + (_time * 60); //x Minutes

//if(_bad) then { _time = time + 1100; } else { _time = time + (15 * 60); };

if(_time <= 0) then { _time = time + (15 * 60); hintC "Rapporter le bug à un Admin: JAIL_FALLBACK_15, temps de zéro!"; };

if(count _ret > 0) then { life_bail_amount = (_ret select 3); } else { life_bail_amount = 4000; /*_time = time + (10 * 60);*/ };
_esc = false;
_bail = false;
...
[_bad,_time] spawn
{
    ...
    life_bail_amount = life_bail_amount * 5; // Ajout
